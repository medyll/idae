...
each role can use Copilot Kanban for task management.the available commands are `kanban_create`, `kanban_update`, `kanban_get`, and `kanban_reset`. If task is in a sprint, the name of the sprint must be mentioned in the kanban task title in parenthesis (e.g., "Sprint 1") before the title.
and description.
