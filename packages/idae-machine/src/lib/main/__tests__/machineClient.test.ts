/**
 * Client integration test: machine.init → machine.boot → store/logic accessible
 * and server schema loading via boot({ sync: { databaseHost } }) → same guarantees.
 *
...
describe('Machine — init + boot (fake-indexeddb)', () => {
	...
	it('machine.store is a function after boot', async () => {
...
describe('Machine — boot with remote schema (mocked fetch)', () => {
	...
	it('boot fetches model and starts machine', async () => {
		const m = new Machine();
	...
	it('boot → collection(vehicle) accessible', async () => {
		const m = new Machine();
		m.init({ org: 'test', domain: 'fetchschema2', version: 1, sync: { databaseHost: 'http://localhost:3000' } });
		await m.boot();
	...
	it('boot → logic.collection(vehicle) resolves', async () => {
		const m = new Machine();
	...
	it('boot resolves without error on subsequent calls', async () => {
