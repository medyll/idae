...
describe('DataList data operations', () => {
	...
	beforeEach(() => {
		resetIndexedDB();
		m = createStartedMachine(uniqueDbName('datalist'));
	});
	...
	describe('getAll', () => {
		it('returns all records for a valid collection', async () => {
			await m.store['vehicle'].create({ license_plate: 'A1', model: 'Civic', brand: 'Honda', status: 'available' });
			await m.store['vehicle'].create({ license_plate: 'B2', model: 'Corolla', brand: 'Toyota', status: 'available' });
			const items = m.store['vehicle'].getAll();
	...
	describe('where', () => {
		it('filters records by field value', async () => {
			await m.store['vehicle'].create({ license_plate: 'A1', model: 'Civic', brand: 'Honda', status: 'available' });
			await m.store['vehicle'].create({ license_plate: 'B2', model: 'Focus', brand: 'Ford', status: 'rented' });
			const filtered = m.store['vehicle'].where({ status: 'available' });
	...
	describe('sort', () => {
		it('sorts items ascending', async () => {
			await m.store['vehicle'].create({ license_plate: 'C3', model: 'Zebra', brand: 'Z', status: 'available' });
			await m.store['vehicle'].create({ license_plate: 'A1', model: 'Alpha', brand: 'A', status: 'available' });
			await m.store['vehicle'].create({ license_plate: 'B2', model: 'Middle', brand: 'M', status: 'available' });
		...
		it('supports multi-sort chain', async () => {
			await m.store['vehicle'].create({ license_plate: 'A1', model: 'B', brand: 'X', status: 'car' });
			await m.store['vehicle'].create({ license_plate: 'B2', model: 'A', brand: 'Y', status: 'car' });
			await m.store['vehicle'].create({ license_plate: 'C3', model: 'C', brand: 'Z', status: 'truck' });
	...
	describe('group', () => {
		it('groups items by field value', async () => {
			await m.store['vehicle'].create({ license_plate: 'A1', model: 'Civic', brand: 'Honda', status: 'available' });
			await m.store['vehicle'].create({ license_plate: 'B2', model: 'Accord', brand: 'Honda', status: 'rented' });
			await m.store['vehicle'].create({ license_plate: 'C3', model: 'F-150', brand: 'Ford', status: 'available' });
	...
	describe('pagination', () => {
		it('slices items by page size', async () => {
			for (let i = 0; i < 10; i++) {
				await m.store['vehicle'].create({ license_plate: `P${i}`, model: `Model ${i}`, brand: 'Test', status: 'available' });
			}
			const all = m.store['vehicle'].getAll();
