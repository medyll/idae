...
export class MachineSchemeFieldForge<T extends Record<string, unknown>> {
	 ...
	 * @param {MachineSchemeValues<T>=} collectionValues Optional collection values instance.
	 */
	constructor(
		collection: TplCollectionName,
		fieldName: keyof T | string,
		...
		this.#collectionValues = collectionValues ?? new MachineSchemeValues(collection, machineDb);
	...
	get htmlInputType(): string | 'text' | 'area' {
		if (this.forge?.fieldType === 'image') return 'file';
