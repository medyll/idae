...
async function seedMenuFixtures(userId: string): Promise<void> {
	...
	// Seed panel pref so widget appears in the panel zone menu (new policy: unset = hidden)
	await machine.collection('appuser_prefs').create({ code: `${userId}:app_panel.widget`, value: true });
}
...
describe('RecentPanel', () => {
	...
	it('lists a recent widget visit grouped under its appscheme_base label', async () => {
