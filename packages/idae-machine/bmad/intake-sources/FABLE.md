...
- [x] e2e parcours principal : login → explorer → fiche → diagram (remplace app.spec.ts obsolète). Écrit + **exécuté 2026-06-10** : `src/e2e/parcours.spec.ts` (app.spec.ts supprimé), creds demo admin/admin123. Stack montée (mongo:27017 + seed demo + server:7842) → **8 passed, 1 skipped, RBAC 5/5**. Le test clé `explorer → fiche → diagram via record action buttons` valide FicheToolbar→ButtonAction de bout en bout. Deux fixes au passage : (1) sélecteur record scopé `explorer-content .data-list-link` (le nav sidebar partage la classe `.data-list-link`, `.first()` non scopé chopait un lien de nav) ; (2) test `diagram via hash direct` passé en `test.fixme` — heurte la **deep-link reload race** (record résolu avant hydratation, cf. §Phase 3 backlog), record bien présent en Mongo, fix = différer le 1er dispatch router, hors scope Phase 2.
...
**Issue découverte (backlog) — deep-link reload race.** ✅ RÉSOLU 2026-06-11. Deux couches : (1) le router tirait **avant** montage de la zone `main` (`data-target-zone`) → frame jamais montée, page blanche ; (2) `buildGraph` faisait un `machine.collection().get()` one-shot avant fin d'hydratation → « Record vehicle:1 not found ». Fix : `MachineRouter.waitForZone()` (MutationObserver + timeout 15 s) — `mountFn` attend la zone au lieu de dropper la navigation ; `diagramUtils.buildGraph` retry le root record après `machine.warmup([collection])` sur cache froid. `test.fixme` « diagram via hash direct » réactivé → e2e **9/9** (parcours 5 + rbac 4, plus aucun skip). check 0/0, unit 585/585 (+3 tests `waitForZone`, pragma jsdom sur machineRouter.test.ts).

### Phase 4 — Transport cleanup (taille M, indépendante — peut s'intercaler)

- [x] Refactor push qoolie sur `idae-api` `parseStream`/`SseStream` — le mode streaming est livré côté idae-api, plus de bloquant. Fait 2026-06-11 : `qoolie/src/lib/push/SSEListener.ts` réécrit en `fetch` + `ReadableStream` (au lieu d'`EventSource` natif) — `Authorization: Bearer` au lieu de token en query string, parsing de frames `data: <json>\n\n` réimplémenté localement (même logique que `parseStream(..., 'sse')`/`SseStream`, sans dépendance runtime sur `@medyll/idae-api` dont le seul export `.` réexporte aussi `server/IdaeApi.js` — express/mongoose/etc, impropre à un bundle client). `isConnected()` piloté par état interne (plus `EventSource.readyState`), reconnect/timeout/setToken conservés. Tests réécrits avec mock `global.fetch` + `ReadableStream` contrôlé. qoolie tests 217/217, idae-machine check 0/0 + tests 582/582. **Phase 4 terminée (3/3).**
- [x] Câbler `idae-socket` comme transport push **dans qoolie** (`SocketIOListener`, `protocol: 'socketio'`, peer dep optionnelle — cf. §3.7) ; idae-machine ne fait que configurer ; `machine.socket` réexpose l'instance qoolie. Supprimer `RealtimeClient`. Un seul transport actif. Fait 2026-06-11 : `qoolie/src/lib/push/SocketIOListener.ts` (implements `PushListener`, wrap `EventDataClientInstance` via import dynamique, mappe `data:created/updated/deleted/restored` → `ServerChange`, `getClient()`) ; `ServerPushListener` sélectionne via `protocol: 'socketio'` + `getListener()` ; `SyncController.getPushListener()` ; `Qoolie.destroy()` stoppe le push ; `PushConfig.collections`/`listener` (seam DI) ajoutés ; `@medyll/idae-socket` en devDep + peerDep optionnelle. idae-machine : `MachineSocket.ts` supprimé, `machine.socket` getter relit l'instance via `sync.getPushListener()→getListener()→getClient()`. Bug fix au passage : interface legacy `SyncController` dans `qoolie/types.ts` (doublon de la classe) manquait les nouvelles méthodes — ajoutées. check 0/0, qoolie tests 217/217 (+8 SocketIOListener), idae-machine tests 582/582.
- [x] Supprimer `MachineMultiBase` / `MachineApi` / `RealtimeClient` (présumés morts, réhabilitation jamais démontrée). Fait 2026-06-11 : suppression `main/api/{MachineApi,RealtimeClient,errors,index}.ts` + tests, `main/multi-base/` entier ; `main/index.ts` ne ré-exporte plus que `AppScheme`/`ViewFieldDef`/`ViewFields` depuis `api/types.ts` (consommé par Breadcrumb + barrel public). check 0/0, tests 582/582 (-27, specs morts retirés).
