...
export const createIdbqlState = (idbBase: IdbqlIndexedCore) => {
	...
	let qolie = function dbQuery(collection: keyof typeof collections) {
		if (!collections[collection]) throw new Error(`Collection ${collection} not found`);
		return {
...
export class CollectionState<T> {
	 ...
	 */
	get where() {
		return (qy: Where<T>, options?: ResultsetOptions) => this._where(qy, options);
	}
	 ...
	 */
	get get(): T | undefined {
		return (value: any, pathKey: string = 'id'): T | undefined => {
	...
	async update(keyPathValue: string | number, data: Partial<T>): Promise<boolean | undefined> {
		if (this.idbBase && this.testIdbql(this.collectionName)) {
			return (await (this.idbBase[this.collectionName] as CollectionCore).update(
				keyPathValue,
				data
	...
	async updateWhere(where: Where<T>, data: Partial<T>): Promise<boolean | undefined> {
		if (this.idbBase && this.testIdbql(this.collectionName)) {
			return (await this.idbBase[this.collectionName].updateWhere(where, data)) as Promise<
				boolean | undefined
			>;
		}
		return undefined;
	...
	async add(data: T): Promise<T | undefined> {
		if (this.idbBase && this.testIdbql(this.collectionName)) {
			return (await this.idbBase[this.collectionName].add<T>(data)) as Promise<T | undefined>;
		}
		return undefined;
	}
