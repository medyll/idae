# S1-02 — Créer `types.ts` — extraire tous les types
**Effort:** S | **Sprint:** 1 | **Dépend de:** —

## Contexte
Tous les types et interfaces sont actuellement définis au début de `Stator.ts`. Il faut les déplacer dans un fichier dédié `types.ts` et enrichir `AugmentedState<T>` avec les signatures des nouvelles méthodes `subscribe()` et `batch()`.

## Fichier à CRÉER
`src/lib/stator/types.ts`

## Contenu EXACT du fichier `types.ts` à créer
...
Supprimer les déclarations de types du début du fichier (les 6 blocs `type`/`interface` : `Primitive`, `StateWrapper`, `StateChangeHandler`, `AugmentedState`, `HandlerContext`) et les remplacer par un seul import :

```typescript
import type { Primitive, StateWrapper, StateChangeHandler, AugmentedState, HandlerContext } from './types.js';
```

**Important :** utiliser l'extension `.js` dans l'import (convention ESM / SvelteKit).

## Acceptance Criteria
- [ ] Fichier `src/lib/stator/types.ts` créé avec le contenu ci-dessus
- [ ] `AugmentedState<T>` contient `subscribe` et `batch` dans sa définition de type
- [ ] `Stator.ts` importe tous ses types depuis `./types.js`
- [ ] Aucun type dupliqué entre `types.ts` et `Stator.ts`
- [ ] `npm run check` (svelte-check + tsc) passe sans erreur de type
- [ ] `npm test` passe (les tests existants ne doivent pas casser)

## Pièges à éviter
- Ne PAS oublier le mot-clé `export` devant chaque `type` et `interface` dans `types.ts`
- Utiliser `import type` (pas `import`) dans `Stator.ts` pour les types uniquement
- L'extension dans l'import doit être `.js` (pas `.ts`) — c'est la convention ESM utilisée dans ce projet
