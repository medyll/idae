...
export class dataOp {
	/** sorting  */
	static sortBy<T = Data>(arr: T[], by: string, sortDir: string = 'asc') {
	...
	static resolveDotPath<T = Data>(object: T, path: ResolverPathType<T>, defaultValue?: any) {
		return (
			path
				.split('.')
				.reduce((r: Record<string, any>, s: any) => (r ? r[s] : defaultValue), object) ?? undefined
		);
	}
