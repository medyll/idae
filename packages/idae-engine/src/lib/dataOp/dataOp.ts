...
export class dataOp {
	...
	static do<T extends object>(options: {
		...
		let result: T[] = options.sort?.arr || options.find?.arr || options.group?.dataList || [];
		...
		return sortArray.sort((a, b) => {
			if ('sort' in sortOptions) {
				...
				for (const [field, direction] of Object.entries(sortOptions.by)) {
					...
					const comparison = this.compareValues(a, b, field as ResolverPathType<T>, sortD);
					if (comparison !== 0) return comparison;
				}
			}
			return 0;
	...
	static find<T extends object = Record<string, unknown>>(options: DataOpFind<T>): T[] {
		...
		return arr.filter((item: T) => {
			if (field !== '*') {
				const itemValue = this.resolveDotPath(item, field as ResolverPathType<T>);
		...
		return dataList.reduce((result: DataOpGroupResult<T>, currentItem: T) => {
			...
			if (typeof groupField === 'function') {
				...
				const groupValue = fields
					.map((field) => this.resolveDotPath(currentItem, field as ResolverPathType<T>))
	 ...
	 */
	private static compareValues<T>(a: T, b: T, field: ResolverPathType<T>, sortD: number): number {
